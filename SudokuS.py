...
default_board_file ="board.txt"
try:
    #python Sudoku_gui board_file.txt
    if len(sys.argv) == 2:
        text_file = sys.argv[1]
        if os.path.isfile(os.getcwd() + '/' + text_file):
            default_board_file = text_file
    else:
        print("Error try this -> python3 SSudoku board_file.txt")
        print("Using default board file ")
    OLD_board = load_board(default_board_file)
...
#draw number at position col,row
...
nope_sound = wav_file_dir+"buzzer_x.wav"
right_sound = wav_file_dir+"coin2.wav"
start_sound = wav_file_dir+"bubbles.wav"
winner_sound = wav_file_dir+"applause.wav"
